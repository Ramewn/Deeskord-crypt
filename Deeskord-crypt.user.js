...
// @name         ⡒⠮⢑⠂⡐⣉⢄
// @version      1.0
// @description  Deeskord-crypt
// @downloadURL  https://raw.githubusercontent.com/Ramewn/Deeskord-crypt/master/Deeskord-crypt.user.js
// @icon         https://raw.githubusercontent.com/Ramewn/Deeskord-crypt/master/logo.png
...
//FOR PRIVITE PURPOSES, SHOULD NOT BE USED

(function() {

'use strict';

const BlacklistUrl = "pach";
...
const BaseColor = "#84b0e0";
const BaseColorInt = 0x84B0E0;
...
<div class="sdc-overlay">
	<form class="SDC_UNBLOCK sdc-window" style="min-width: 480px">
		...
		<div style="margin-right:20px" >
			<img style="border-radius: 32px;" src="https://raw.githubusercontent.com/Ramewn/Deeskord-crypt/master/logo.png">
		</div>
			<div style="margin-top:25px"><h4>Unlock Database</h4></div>
		</div>
		<h5 style="margin-top:20px">Password</h5>
		<input class="SDC_PASSWORD" style="margin-top:8px;margin-bottom:20px;border-color:#84b0e0" type="password" name="sdc-password">
		<div class="sdc-footer"><button type="button" class="SDC_CANCEL sdc-lnkbtn"><p>Cancel</p></button><button type="button" class="SDC_NEWDB sdc-rbtn" style="margin:0 4px">New DB</button><button type="submit" class="sdc-btn" style="min-width:96px;background-color:#84b0e0">Unlock</button></div>
	</form>
</div>
</div>`,
...
<div class="sdc-overlay">
	<form class="SDC_CREATEDB sdc-window" style="min-width: 480px">
			...
			<div style="margin-top:25px"><h4>Create Database</h4></div>
		</div>
...
var Discord;
var Utils = {
    Log: (message) => { console.log(`%c[⡒⠮⢑⠂⡐⣉⢄] %c${message}`, `color:${BaseColor};font-weight:bold`, "") },
    Warn: (message) => { console.warn(`%c[⡒⠮⢑⠂⡐⣉⢄] %c${message}`, `color:${BaseColor};font-weight:bold`, "") },
    Error: (message) => { console.error(`%c[⡒⠮⢑⠂⡐⣉⢄] %c${message}`, `color:${BaseColor};font-weight:bold`, "") }
};
var DataBase;
var Cache;
var Blacklist;
...
function Init(nonInvasive)
{
    ...
    Object.assign(Utils, {
        ...
        LoadDb: function(callback, failCallback, reload) { (async () => {
            if(!reload) DataBase = await this.StorageLoad('⡒⠮⢑⠂⡐⣉⢄');
            if(DataBase != null) {
                Cache = { keys: {} };
        ...
        SaveDb: async function() {
            ...
            await this.StorageSave('⡒⠮⢑⠂⡐⣉⢄', DataBase);
        },
        saveDbTimeout: null,
        FastSaveDb: function() {
            this.dbChanged = true;
        ...
        DownloadDb: async function(uncompressed) {
            ...
            this.DownloadBlob(uncompressed ? "⡒⠮⢑⠂⡐⣉⢄.json" : "⡒⠮⢑⠂⡐⣉⢄.dat", new Blob([buffer]));
        },
            ...
            Discord.enqueue({
                ...
                message: {
                    ...
                    embed: {
                        ...
                        author: {
                            name: "-----SYSTEM MESSAGE-----",
                            icon_url: "https://raw.githubusercontent.com/Ramewn/Deeskord-crypt/master/logo.png",
                        },
                        description: sysmsg,
                        footer: {
...
async function decryptMessage(message, payload) {
    ...
    if(payloadBuffer.byteLength === 16) {
        message.content = "\u2063:small_blue_diamond:"; //invisible separator at the end to make the emoji smaller
    }
    else {
        let content;
        try {
		...
		message.content = "\u2063:small_blue_diamond:" + content;
       // message.content = content.replace(/^/gm, "\u2063:small_blue_diamond: "); //bad for code blocks
...
async function processEmbeds(message) {
    ...
    if(embed.footer == null || (embed.footer.text !== "⡒⠮⢑⠂⡐⣉⢄" && embed.footer.text !== "\u{1D61A}\u{1D62A}\u{1D62E}\u{1D631}\u{1D62D}\u{1D626}\u{1D60B}\u{1D62A}\u{1D634}\u{1D624}\u{1D630}\u{1D633}\u{1D625}\u{1D60A}\u{1D633}\u{1D63A}\u{1D631}\u{1D635}")) return;
